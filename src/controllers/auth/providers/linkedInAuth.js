...
const linkedInAuth = async (req, res) => {
    try {
        ...
        logger_1.logger.info("LinkedIn OAuth: Starting token exchange process");
        /**
         ...
         */
        const tokenResponse = await axios_1.default.post("https://www.linkedin.com/oauth/v2/accessToken", 
        ...
        logger_1.logger.info("LinkedIn OAuth: Access token obtained successfully");
        /**
        ...
        logger_1.logger.info(`LinkedIn OAuth: Retrieved profile for ${linkedinUser.email}`);
        logger_1.logger.info(`LinkedIn OAuth: Retrieved profile for ${JSON.stringify(profileResponse.data)}`);
        /**
        ...
        const existingUsers = await database_setup_1.db
            ...
            .where((0, drizzle_orm_1.eq)(database_setup_1.users.email, linkedinUser.email))
            .limit(1);
        let user;
        if (existingUsers.length > 0) {
            ...
            logger_1.logger.info(`LinkedIn OAuth: Updated existing user: ${user.email}`);
        }
        else {
            // New user - create account with LinkedIn data
            const newUsers = await database_setup_1.db
                ...
                .returning();
            user = newUsers[0];
            logger_1.logger.info(`LinkedIn OAuth: Created new user: ${user.email}`);
        }
        /**
        ...
        logger_1.logger.error("LinkedIn OAuth error: " + error);
